...
from .get_atten import get_local
get_local.activate()
int_classes = int
string_classes = str
...
class Attention(nn.Module):
    ...
    @get_local('attn')
    def forward(self, x, atten=None, i = None):
        B, N, C = x.shape
...
class Block(nn.Module):
    ...
    def forward(self, x, atten = None, i = None):
        # if atten != None:
        #     x = x + self.drop_path(self.attn(self.norm1(x), atten, i))
        # else:
...
class PatchEmbed_high(nn.Module):
    """ Image to Patch Embedding with overlapping patches
    """
    def __init__(self, img_size=224, patch_size=16, stride_size=20, in_chans=3, embed_dim=768, ratio = None):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        stride_size_tuple = to_2tuple(stride_size)
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.pos_embed_1 = nn.Parameter(torch.zeros(1, 220 + 1, embed_dim).cuda())
        self.cam_num = camera
        self.view_num = view
    ...
    def forward_features(self, x,  camera_id, view_id, atten = None):
        ...
        if self.local_feature:
            ...
            return x[:, 0]
    def forward(self, x, cam_label=None, view_label=None, atten = None):
            if atten != None:
                x = self.forward_features(x,  cam_label, view_label, atten)
...
def vit_base_patch16_224_TransReID(img_size=(384, 384), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, ratio = None, **kwargs):
    model = TransReID(
